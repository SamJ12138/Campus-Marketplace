...
# Auth schema diagnostic
@app.get("/health/auth")
async def health_auth():
    """Check if passwordless auth schema is correctly set up."""
    checks = {}
    try:
        async with app.state.db_session() as db:
            # Check enum values
            result = await db.execute(text(
                ...
                "WHERE t.typname = 'email_verification_purpose'"
            ))
            enum_values = [row[0] for row in result.fetchall()]
            checks["enum_values"] = enum_values
            checks["login_code_exists"] = "login_code" in enum_values

            # Check attempt_count column
            result = await db.execute(text(
                "SELECT column_name FROM information_schema.columns "
                "WHERE table_name = 'email_verifications' "
                "AND column_name = 'attempt_count'"
            ))
            checks["attempt_count_column"] = result.fetchone() is not None
            ...
            row = result.fetchone()
            checks["password_hash_nullable"] = row[0] == "YES" if row else "column_missing"

            # Check token_hash unique constraint
            result = await db.execute(text(
                "SELECT constraint_name FROM information_schema.table_constraints "
                "WHERE table_name = 'email_verifications' "
                "AND constraint_type = 'UNIQUE'"
            ))
            checks["unique_constraints"] = [row[0] for row in result.fetchall()]

            # Count existing login code records
            result = await db.execute(text(
                "SELECT COUNT(*) FROM email_verifications WHERE purpose = 'login_code'"
            ))
            row = result.fetchone()
            checks["login_code_records"] = row[0] if row else "query_failed"

    except Exception as e:
        checks["error"] = str(e)

    all_ok = checks.get("login_code_exists") and checks.get("attempt_count_column")
    return {"status": "ok" if all_ok else "missing_schema", "checks": checks}
