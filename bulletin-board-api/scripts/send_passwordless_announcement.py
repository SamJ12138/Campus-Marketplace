#!/usr/bin/env python
"""One-time script: notify existing password-based users about passwordless auth.

Usage:
    python scripts/send_passwordless_announcement.py --dry-run        # preview only
    python scripts/send_passwordless_announcement.py --to user@edu    # send to one user
    python scripts/send_passwordless_announcement.py                  # send to all
"""
import argparse
import asyncio
import logging
...
from app.services.email_templates import passwordless_announcement_email
...
async def main(dry_run: bool = False, to_email: str | None = None, skip_test: bool = False):
    ...
    async with async_session() as db:
        query = select(User).where(User.password_hash.isnot(None))
        if to_email:
    ...
    logger.info("Found %d user(s) to notify.", len(users))
    ...
    for u in users:
        ...
        html, text = passwordless_announcement_email(display)
        success = await email_service.send_email(
            to_email=u.email,
            subject="Signing in just got easier",
            html_content=html,
            text_content=text,
        )
        if success:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Send passwordless auth announcement")
