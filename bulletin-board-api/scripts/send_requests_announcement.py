#!/usr/bin/env python
"""Announcement email: new Requests feature on GimmeDat.

Usage:
    python scripts/send_requests_announcement.py --dry-run        # preview only
    python scripts/send_requests_announcement.py --to user@edu    # send to one user
    python scripts/send_requests_announcement.py --skip-test       # skip test accounts
    python scripts/send_requests_announcement.py                  # send to all
"""
import argparse
import asyncio
import logging
...
from app.services.email_templates import requests_feature_email
...
SUBJECT = "New on GimmeDat: Post what you need"
...
async def main(dry_run: bool = False, to_email: str | None = None, skip_test: bool = False):
    ...
    logger.info("Found %d user(s) to email.", len(users))
    ...
    for u in users:
        ...
        html, text = requests_feature_email(display, settings.primary_frontend_url)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Send requests feature announcement email")
